...
const Movies = () => {
  ...
  const [filter, setFilter] = useState('popular'); // Add a new state to handle filter
  ...
  useEffect(() => {
    getContent();
  }, [page, filter]); // Update the dependencies to include "filter"
  ...
  const handleFilterChange = (event) => {
    ...
    setPage(1);
    setContent([]); // Clear the contents when filter changes
  }
  return (
    <div>
      {isLoading ? (
